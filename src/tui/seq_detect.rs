...
impl SeqDetector {
    ...
    fn check(&mut self) -> Option<EasterEgg> {
        let egg = if self.tail_matches("iddqd") {
            Some(EasterEgg::GodMode)
        } else if self.tail_matches("idkfa") {
            Some(EasterEgg::FillNotes)
        } else if self.tail_matches("xyzzy") {
            Some(EasterEgg::Xyzzy)
        } else if self.tail_matches("sudo") {
            Some(EasterEgg::Sudo)
        } else if self.tail_matches("help") {
            Some(EasterEgg::Help)
        } else if self.tail_matches("fortytwo") {
            Some(EasterEgg::FortyTwo)
        } else if self.tail_matches("zweiundvierzig") {
            Some(EasterEgg::FortyTwo)
        } else if self.tail_matches("matrix") {
            Some(EasterEgg::MatrixMode)
        } else {
            None
        };
        if egg.is_some() {
...
mod tests {
    ...
    #[test]
    fn iddqd() {
        assert_eq!(detect("iddqd"), Some(EasterEgg::GodMode));
    }
    #[test]
    fn idkfa() {
        assert_eq!(detect("idkfa"), Some(EasterEgg::FillNotes));
    }
    #[test]
    fn xyzzy() {
        assert_eq!(detect("xyzzy"), Some(EasterEgg::Xyzzy));
    }
    #[test]
    fn sudo() {
        assert_eq!(detect("sudo"), Some(EasterEgg::Sudo));
    }
    #[test]
    fn help() {
        assert_eq!(detect("help"), Some(EasterEgg::Help));
    }
    #[test]
    fn fortytwo_en() {
        assert_eq!(detect("fortytwo"), Some(EasterEgg::FortyTwo));
    }
    #[test]
    fn fortytwo_de() {
        assert_eq!(detect("zweiundvierzig"), Some(EasterEgg::FortyTwo));
    }
    #[test]
    fn matrix() {
        assert_eq!(detect("matrix"), Some(EasterEgg::MatrixMode));
    }
    ...
    fn konami_wrong_key_resets() {
        ...
        let mut d = SeqDetector::default();
        for dir in [Up, Up, Down, Left] {
            d.push_direction(dir);
        }
