...
fn render_congrats(
    ...
    if tick % 6 >= 4 { return Ok(()); }

    let art_w = CONGRATS_ART.iter()
        .map(|l| l.chars().count())
        .max()
        .unwrap_or(47) as u16;
    ...
    let fg = if (tick / 6) % 2 == 0 { colors.digit_given } else { colors.ui_text };
...
fn in_bounds(col: i16, row: i16) -> bool {
    col >= CLIP_COL_MIN && col <= CLIP_COL_MAX
        && row >= CLIP_ROW_MIN && row <= CLIP_ROW_MAX
}
...
mod tests {
    ...
    fn render_firework_congrats_phase_produces_output() {
        ...
        assert!(s.contains("\\ V /  | |_| |"), "expected figlet art fragment in output");
    }
