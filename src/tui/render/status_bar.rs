...
pub fn render_panel(
    ...
    let error_state_label = if error_mode {
        strings.toggle_on
    } else {
        strings.toggle_off
    };
    let mut rows: Vec<(String, Color, bool)> = vec![
        ...
        (String::new(), t, false),
        (
            strings.panel_errors.replacen("{}", error_state_label, 1),
            t,
            false,
        ),
        (
            strings
                .panel_count
                .replacen("{}", &errors_shown.to_string(), 1),
            t,
            false,
        ),
        (String::new(), t, false),
        (
            strings
                .panel_filled
                .replacen("{}", &filled_count.to_string(), 1),
            t,
            false,
        ),
    ...
    if let Some((name, explanation)) = hint_text {
        ...
        rows.extend(vec![
            ...
            (strings.ctrl_mouse.into(), d, false),
...
mod tests {
    ...
    fn word_wrap_splits_on_width() {
        let lines = word_wrap(
            "Only 5 fits in this cell because all others are eliminated.",
            20,
        );
